...
def home(request):
    ...
    store = Store.objects.filter(is_active=True)
    return render(request, 'server/home.html', {'banners': banners, 'first': first, 'second': second, 'third': third, 'fourth': fourth, 'featured_product': featured_product, 'best_sellers': best_sellers, 'mid_banner': mid_banner, 'store': store})
